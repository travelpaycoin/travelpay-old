...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tpc address (e.g. %1)").arg("idaeAtzaCGnJ5SUPXKoFxtgzbvv5nfBSDn"));
#endif
...
{
    // return if URI is not valid or is no TPC: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert tpc:// to tpc:
    //
    //    Cannot handle this later, because tpc:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open Travelpaycoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TPC.lnk";
}
...
{
    return GetAutostartDir() / "tpc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a tpc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the tpc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add tpc app to startup item list
