...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered tpc address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TPC.").arg(GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal tpc:URI (tpc:XyZ...?message=example)
            {
...
static void NotifyxTPCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyxTPCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyxTPCReset.connect(boost::bind(NotifyxTPCReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyxTPCReset.disconnect(boost::bind(NotifyxTPCReset, this));
}
