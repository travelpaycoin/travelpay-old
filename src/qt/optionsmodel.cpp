...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTPCAmount"))
        settings.setValue("nAnonymizeTPCAmount", 1000);

    nAnonymizeTPCAmount = settings.value("nAnonymizeTPCAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTPCAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeTPCAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in tpc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTPCAmount:
            return QVariant(nAnonymizeTPCAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTPCAmount:
            nAnonymizeTPCAmount = value.toInt();
            settings.setValue("nAnonymizeTPCAmount", nAnonymizeTPCAmount);
            emit anonymizeTPCAmountChanged(nAnonymizeTPCAmount);
            break;
        case CoinControlFeatures:
