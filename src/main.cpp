...
#if defined(NDEBUG)
#error "TPC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uTPC) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Tpc Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : xTPC with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : xTPC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: xTPC spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TPC
...
void RecalculateXIONSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from xTPC supply
...
bool RecalculateTPCSupply(int nHeightStart)
{
...
/** FornaxA: Tpc accepts PoS during PoW phase 
 *
 *   if (pindex->nHeight <= Params().LAST_POW_BLOCK() && block.IsProofOfStake())
 *       return state.DoS(100, error("ConnectBlock() : PoS period not active"),
 *           REJECT_INVALID, "PoS-early");
 */
/** cevap: Tpc accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 */ 
/** cevap: Tpc accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 ...
 */
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTPCSupply(Params().Zerocoin_StartHeight());
    }

    //Track xTPC money supply in the block index
    if (!UpdateXIONSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new xTPC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s xTPCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert Tpc to xTPC
...
/*    if (block.GetBlockTime() > Params().Zerocoin_StartTime()) {
        ...
        // TPCTor - disable reject block as our blocks are in version 8 since block 1
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.GetBlockTime() > GetAdjustedTime() + (block.IsProofOfStake() ? 30 : 7200)) // 3 minute future drift for PoS
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent xTPC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of xTPC serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake() && !( (block.GetBlockTime()==block.vtx[1].nTime && ((block.vtx[1].nTime&STAKE_TIMESTAMP_MASK) ==0 ) ) ) )
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d xTPC mints and %d xTPC spends\n", __func__, nMints, nSpends);
...
string GetWarnings(string strFor)
{
    ...
    if (fLargeWorkForkFound) {
        nPriority = 2000;
        strStatusBar = strRPC = _("Warning: The network does not appear to fully agree! Some diggers appear to be experiencing issues.");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TPC: We use certain sporks during IBD, so check to see if they are
