...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
	...
	txCoinStake.nTime = pblock->nTime;        
        ...
        if (nSearchTime >= nLastCoinStakeSearchTime) {
            unsigned int nTxNewTime = txCoinStake.nTime;
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // xTPC spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent xTPC spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This xTPC serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TPCDigger : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TPCDigger : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TPCDigger started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tpc-digger");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //Stake miner main
        if (fProofOfStake) {
            LogPrintf("CPUDigger : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());

            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("BitcoinDigger(): Signing new block failed \n");
                continue;
            }

            LogPrintf("CPUDigger : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
        ...
        LogPrintf("Running TPCDigger with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinDigger(void* parg)
{
    ...
    LogPrintf("ThreadBitcoinDigger exiting\n");
}
